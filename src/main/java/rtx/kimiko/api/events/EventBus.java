        ...
        for (Class<?> clazz2 = object.getClass(); clazz2 != null && clazz2 != Object.class; clazz2 = clazz2.getSuperclass()) {
            for (Method method : clazz2.getDeclaredMethods()) {
                ...
                try {
                    ...
                    Kimiko.LOGGER.error("[EventBus] Failed to register handler {}.{}", new Object[]{clazz2.getSimpleName(), method.getName(), exception});
                }
            }
        }
    }
        ...
        public void invoke(Event event) {
            try {
                ...
                if (EventBus.REPORTED_FAILURES.add(string)) {
                    Kimiko.LOGGER.error("[EventBus] Exception in event handler {} for {} (further identical errors suppressed)", this.owner.getClass().getSimpleName(), event.getClass().getSimpleName(), throwable);
                }
            }
        }
    }
