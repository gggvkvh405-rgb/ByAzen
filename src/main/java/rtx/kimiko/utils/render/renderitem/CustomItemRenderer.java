package rtx.kimiko.utils.render.renderitem;
...
import rtx.kimiko.mixin.accessor.ItemLayerRenderStateAccessor;
import rtx.kimiko.mixin.accessor.ItemStackRenderStateAccessor;
...
import rtx.kimiko.utils.render.render2d.effecticon.PoseKey;
import rtx.kimiko.utils.render.renderitem.BuiltRenderItem;
import rtx.kimiko.utils.render.renderitem.CachedItemGeometry;
import rtx.kimiko.utils.render.renderitem.CachedItemQuad;
import rtx.kimiko.utils.render.renderitem.CustomItemRenderState;
import rtx.kimiko.utils.render.renderitem.ItemTexture;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltRenderItem builtRenderItem) {
        ...
        try {
            ...
            int n = ((GuiRenderStateLayerAccessor)guiRenderState).kimiko_getLayerSerial();
            ...
            Kimiko.LOGGER.warn("[RenderItem] Failed to submit item: {}", (Object)builtRenderItem.stack().getName().getString(), (Object)runtimeException);
        }
    }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_item_params", 136, 65536L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_item_params", 128, byteBuffer);
            this.paramsDirty = false;
    ...
    private CachedItemGeometry buildGeometry(MinecraftClient minecraftClient, ItemStack itemStack, int n) {
        ...
        ItemRenderState.LayerRenderState[] layerRenderStateArray = itemStackRenderStateAccessor.kimiko_getLayers();
        int n2 = Math.min(itemStackRenderStateAccessor.kimiko_getActiveLayerCount(), layerRenderStateArray.length);
        for (int i = 0; i < n2; ++i) {
    ...
    private Matrix4f layerTransform(ItemLayerRenderStateAccessor itemLayerRenderStateAccessor) {
        ...
        Transformation transformation = itemLayerRenderStateAccessor.kimiko_getItemTransform();
    ...
    private void collectLayerGeometry(List<RawItemQuad> list, Bounds bounds, ItemRenderState.LayerRenderState layerRenderState) {
        ...
        ItemRenderState.Glint glint = itemLayerRenderStateAccessor.kimiko_getFoilType();
        boolean bl = glint != ItemRenderState.Glint.NONE;
        int[] nArray = itemLayerRenderStateAccessor.kimiko_getTintLayers();
        if (nArray == null) {
            nArray = EMPTY_TINTS;
        }
        Matrix4f matrix4f = this.layerTransform(itemLayerRenderStateAccessor);
        boolean bl2 = itemLayerRenderStateAccessor.kimiko_getUsesBlockLight();
        if (bl2) {
    ...
    private boolean hasSpecialRenderer(ItemRenderState.LayerRenderState layerRenderState) {
        return layerRenderState != null && ((ItemLayerRenderStateAccessor)layerRenderState).kimiko_getSpecialRenderer() != null;
    }
