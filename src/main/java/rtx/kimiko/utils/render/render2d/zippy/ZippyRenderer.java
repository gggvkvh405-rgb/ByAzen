...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltZippy builtZippy) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new ZippyRenderState(matrix3x2f, builtZippy2));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_zippy_params", 136, 12288L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedZippy);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_zippy_params", 128, byteBuffer);
            this.paramsDirty = false;
