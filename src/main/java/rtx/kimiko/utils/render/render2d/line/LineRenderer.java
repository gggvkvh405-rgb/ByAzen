...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltLine builtLine) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new LineRenderState(matrix3x2f, builtLine, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_line_params", 136, 65536L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedLines);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_line_params", 128, byteBuffer);
            this.paramsDirty = false;
