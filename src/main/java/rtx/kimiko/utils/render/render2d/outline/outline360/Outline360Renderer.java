...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltOutline360 builtOutline360) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new Outline360RenderState(matrix3x2f, builtOutline3602));
        }
    ...
    private void ensureBuffers() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ...
            ByteBuffer byteBuffer2 = memoryStack.calloc(49152);
            this.buildUniformData(byteBuffer, byteBuffer2, this.preparedOutlines);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_outline_360_params", 128, byteBuffer);
            this.rangesBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_outline_360_ranges", 128, byteBuffer2);
            this.paramsDirty = false;
        }
    }
    ...
    private boolean ensureWritableBuffers() {
        ...
        this.closeBuffers();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_outline_360_params", 136, 16384L);
            this.rangesBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_outline_360_ranges", 136, 49152L);
            return true;
        }
        catch (RuntimeException runtimeException) {
            this.closeBuffers();
