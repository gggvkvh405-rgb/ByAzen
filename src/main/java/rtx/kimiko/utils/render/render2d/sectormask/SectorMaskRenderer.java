...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltSectorMask builtSectorMask) {
        ...
        try {
            Matrix3x2f matrix3x2f = rtx.kimiko.utils.render.render2d.Render2DCoordinateSpace.pose(drawContext);
            ((rtx.kimiko.mixin.accessor.GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new SectorMaskRenderState(matrix3x2f, builtSectorMask, rtx.kimiko.utils.render.scissor.ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_sector_mask_params", 136, 640L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedMasks);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_sector_mask_params", 128, byteBuffer);
            this.paramsDirty = false;
