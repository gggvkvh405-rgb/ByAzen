...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltRectangle builtRectangle) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new DefaultRectangleRenderState(matrix3x2f, builtRectangle2, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensurePageBuffer(int n) {
        ...
        if (gpuBuffer == null || gpuBuffer.isClosed() || gpuBuffer.size() < 64512L) {
            ...
            try {
                gpuBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_rect_default_params", 136, 64512L);
            }
            catch (RuntimeException runtimeException) {
