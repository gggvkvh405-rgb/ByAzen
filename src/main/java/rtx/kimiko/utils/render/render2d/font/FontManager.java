...
implements AutoCloseable {
    ...
    private Font loadFont(String string, Font font) {
        try (InputStream inputStream = FontManager.class.getResourceAsStream(FONT_ROOT + string);){
            if (inputStream == null) {
                Kimiko.LOGGER.debug("[FontRenderer] Font asset not found, using fallback: {}", (Object)string);
                return font;
            }
            ...
            Kimiko.LOGGER.warn("[FontRenderer] Failed to load font asset: {}", (Object)string, (Object)exception);
            return font;
        }
    }
