...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltOutline builtOutline) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new DefaultOutlineRenderState(matrix3x2f, builtOutline2, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_outline_default_params", 136, 64512L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedOutlines);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_outline_default_params", 128, byteBuffer);
            this.paramsDirty = false;
