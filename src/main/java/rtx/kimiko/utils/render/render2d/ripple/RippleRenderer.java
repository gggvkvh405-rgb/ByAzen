...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltRipple builtRipple) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new RippleRenderState(matrix3x2f, builtRipple, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_ripple_params", 136, 40960L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedRipples);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_ripple_params", 128, byteBuffer);
            this.paramsDirty = false;
