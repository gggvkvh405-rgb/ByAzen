...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltShape builtShape) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).kimiko_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new ShapeRenderState(matrix3x2f, builtShape2, ScissorUtil.current(), blurCapture));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_shape_params", 136, 18688L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedShapes);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_shape_params", 128, byteBuffer);
            this.paramsDirty = false;
