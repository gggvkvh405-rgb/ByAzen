...
import rtx.kimiko.api.ui.window.GuiShatterAnimation.Remap;
...
public final class GuiLayerBlurRenderer {
        ...
        block6: {
            ...
            try {
                ...
                if (worldDepthCopy == null) {
                    ...
                    worldDepthCopy = gpuDevice.createTexture(() -> "kimiko:gui_share_world_depth", 5, TextureFormat.DEPTH32, framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
                ...
                Kimiko.LOGGER.warn("[GuiShare] world depth snapshot failed", throwable);
            }
        }
    }
    ...
    public static void snapshotHandDepth() {
        block6: {
            ...
            try {
                ...
                Kimiko.LOGGER.warn("[GuiShare] hand depth snapshot failed", throwable);
            }
        }
    }
    ...
    public static Framebuffer worldBackdropSource(Framebuffer framebuffer) {
        ...
        try {
            ...
            if (list.isEmpty()) {
                ...
                try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:world_backdrop_warp", sceneSnapshot.getColorAttachmentView(), OptionalInt.of(0));){
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:world_backdrop_warp_slots", sceneSnapshot.getColorAttachmentView(), OptionalInt.of(0));){
    ...
    private static void ensureSceneSnapshotTarget(int n, int n2) {
        ...
        sceneSnapshot = GuiLayerBlurRenderer.destroy(sceneSnapshot);
        try {
            sceneSnapshot = new SimpleFramebuffer("kimiko_world_backdrop_snapshot", n, n2, false);
        }
        catch (Throwable throwable) {
            sceneSnapshot = null;
        }
    ...
    private static void blitPanelsIntoPopupSlots(CommandEncoder commandEncoder) {
        ...
        for (RemoteGuiWorld.PanelQuad panelQuad : list) {
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:gui_share_popup_slot_blit", sceneSnapshot.getColorAttachmentView(), OptionalInt.empty())) {
    ...
    private static boolean ensureTargets(int n, int n2) {
        ...
        GuiLayerBlurRenderer.closeTargets();
        guiFbo = new SimpleFramebuffer("kimiko_gui_capture", n, n2, true);
        tempH = new SimpleFramebuffer("kimiko_gui_capture_h", n, n2, false);
        tempV = new SimpleFramebuffer("kimiko_gui_capture_v", n, n2, false);
        texWidth = n;
        texHeight = n2;
        return true;
    }
    ...
    private static boolean ensureRemoteFbo(int n, int n2) {
        ...
        return (remoteFbo = new SimpleFramebuffer("kimiko_gui_share_capture", n, n2, true)).getColorAttachmentView() != null;
    }
    ...
    private static boolean drawRemoteShards(CommandEncoder commandEncoder, Framebuffer framebuffer, RemoteGuiWorld.PanelQuad panelQuad, Matrix4f matrix4f) {
        ...
        int n = ((rtx.kimiko.api.ui.window.GuiShatterAnimation.State)shatter.state()).buildGeometry(shardVertexData, shatter.progress(), shatter.alpha(), 1.0f, true, remap);
        if (n <= 0 || shardVertexData.remaining() <= 0) {
            return false;
        }
        ...
        commandEncoder.writeToBuffer(shardVertexBuffer.slice(0L, (long)shardVertexData.remaining()), shardVertexData);
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:gui_share_shards", framebuffer.getColorAttachmentView(), OptionalInt.empty());){
    ...
    private static void disableAfterError(Throwable throwable) {
        ...
        captureActive = false;
        GuiLayerBlurRenderer.closeTargets();
        Kimiko.LOGGER.error("[GuiLayerBlur] disabled after error", throwable);
    }
