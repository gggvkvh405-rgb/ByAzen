package rtx.kimiko.utils.render.post.itemoutline;
...
public final class ItemOutlineRenderer {
    ...
    private static void init() {
        ...
        try {
            ...
            dtConfigBuffer = ItemOutlineRenderer.ensureBuffer(dtConfigBuffer, 16, "kimiko:vm_outline_dt_config");
            isoConfigBuffer = ItemOutlineRenderer.ensureBuffer(isoConfigBuffer, 32, "kimiko:vm_outline_iso_config");
        }
        catch (Throwable throwable) {
            disabledAfterError = true;
            isoPipeline = null;
    ...
    private static boolean ensureTargets(int n, int n2) {
        ...
        ItemOutlineRenderer.closeTargets();
        silhouette = new SimpleFramebuffer("kimiko_vm_outline_silhouette", n, n2, true);
        swap = new SimpleFramebuffer("kimiko_vm_outline_swap", n, n2, false);
        targetWidth = n;
        targetHeight = n2;
        return true;
    }
    ...
    private static void isolines(GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, float f, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:vm_outline_iso", gpuTextureView, OptionalInt.empty())) {
    ...
    private static void drainSilhouette(OutlineVertexConsumerProvider outlineVertexConsumerProvider) {
        CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
        RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:vm_outline_clear", silhouette.getColorAttachmentView(), OptionalInt.of(0));
        if (renderPass != null) {
            renderPass.close();
        }
    ...
    private static void distanceTransformH(GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:vm_outline_dt_h", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0));){
    ...
    private static void distanceTransformV(GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, GpuSampler gpuSampler) {
        CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
        ItemOutlineRenderer.writeDtConfig(commandEncoder, simpleFramebuffer);
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:vm_outline_dt_v", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0));){
