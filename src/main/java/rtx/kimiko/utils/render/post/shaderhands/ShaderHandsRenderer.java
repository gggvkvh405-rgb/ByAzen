package rtx.kimiko.utils.render.post.shaderhands;
...
public final class ShaderHandsRenderer {
    ...
    private static void init() {
        ...
        try {
            ...
            kawaseBuffer = ShaderHandsRenderer.ensureBuffer(kawaseBuffer, 16, "kimiko:shader_hands_kawase");
            glassBuffer = ShaderHandsRenderer.ensureBuffer(glassBuffer, 32, "kimiko:shader_hands_glass");
            glowBuffer = ShaderHandsRenderer.ensureBuffer(glowBuffer, 96, "kimiko:shader_hands_glow");
            gaussBuffer = ShaderHandsRenderer.ensureBuffer(gaussBuffer, 224, "kimiko:shader_hands_gauss");
            outlineBuffer = ShaderHandsRenderer.ensureBuffer(outlineBuffer, 96, "kimiko:shader_hands_outline");
            reduceBuffer = ShaderHandsRenderer.ensureBuffer(reduceBuffer, 16, "kimiko:shader_hands_reduce");
            initBuffer = ShaderHandsRenderer.ensureBuffer(initBuffer, 16, "kimiko:shader_hands_bounds_initcfg");
            boundsWriteBuffer = ShaderHandsRenderer.ensureBuffer(boundsWriteBuffer, 16, "kimiko:shader_hands_bounds_write");
        }
    ...
    private static void glowDilate(GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, float f, float f2, float f3, int[] nArray, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, GpuSampler gpuSampler, boolean bl) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_glow_dilate", simpleFramebuffer.getColorAttachmentView(), bl ? OptionalInt.of(0) : OptionalInt.empty())) {
    ...
    private static void glowGauss(GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, float f, float f2, float f3, int[] nArray, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_glow_gauss", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0))) {
    ...
    private static void boundsInit(GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, float f, float f2, int n, GpuTextureView gpuTextureView2) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_bounds_init", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0))) {
    ...
    private static void kawasePass(RenderPipeline renderPipeline, GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, float f, float f2, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_kawase", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0))) {
    ...
    private static void glass(GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, GpuTextureView gpuTextureView4, GpuTextureView gpuTextureView5, int n, float f, float f2, float f3, float f4, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_glass", gpuTextureView, OptionalInt.empty())) {
    ...
    public static void composite(int n, int[] nArray, boolean bl, boolean bl2, int n2, float f, float f2, float f3, boolean bl3, float f4, float f5, float f6, float f7) {
        ...
        try {
            ...
            if (bl) {
                ...
                CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
                try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_plain", framebuffer.getColorAttachmentView(), OptionalInt.empty());){
    ...
    private static void outline(GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, int[] nArray, GpuTextureView gpuTextureView4, GpuTextureView gpuTextureView5, float f, float f2, float f3, boolean bl, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_outline", gpuTextureView, OptionalInt.empty())) {
    ...
    public static void captureScene(boolean bl) {
        ...
        try {
            ...
            GpuSampler gpuSampler = RenderSampler.linear();
            CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
            if (handFbo != null && (renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_clear", handFbo.getColorAttachmentView(), OptionalInt.of(0), handFbo.getDepthAttachmentView(), OptionalDouble.of(1.0))) != null) {
                renderPass.close();
            }
            handReady = false;
            sceneReady = false;
    ...
    public static void updateHandMask() {
        ...
        try {
            ...
            CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:hand_mask_grid", maskGrid.getColorAttachmentView(), OptionalInt.of(0));){
            ...
            if (maskReadBuffer == null || maskReadBuffer.isClosed()) {
                maskReadBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko:hand_mask_read", 9, (long)n2);
            }
    ...
    private static boolean ensureTargets(int n, int n2) {
        ...
        ShaderHandsRenderer.closeTargets();
        sceneCopyTexture = gpuDevice.createTexture(() -> "kimiko:shader_hands_scene_copy", 5, TextureFormat.RGBA8, n, n2, 1, 1);
        sceneCopyTextureView = gpuDevice.createTextureView(sceneCopyTexture);
        sceneA = new SimpleFramebuffer("kimiko_shader_hands_scene_a", n, n2, false);
        sceneB = new SimpleFramebuffer("kimiko_shader_hands_scene_b", n, n2, false);
        handFbo = new SimpleFramebuffer("kimiko_shader_hands_hand", n, n2, true);
        glow = new SimpleFramebuffer("kimiko_shader_hands_glow", n, n2, false);
        glowSwap = new SimpleFramebuffer("kimiko_shader_hands_glow_swap", n, n2, false);
        boundsChain.clear();
        int n3 = n;
        int n4 = n2;
        int n5 = 0;
        boundsChain.add(new SimpleFramebuffer("kimiko_shader_hands_bounds_" + n5, n3, n4, false));
        while (n3 > 1 || n4 > 1) {
            n3 = Math.max(1, n3 / 2);
            n4 = Math.max(1, n4 / 2);
            boundsChain.add(new SimpleFramebuffer("kimiko_shader_hands_bounds_" + ++n5, n3, n4, false));
    ...
    private static void writeBounds(SimpleFramebuffer simpleFramebuffer, float[] fArray) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_bounds_write", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0))) {
    ...
    private static void boundsReduce(GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, int n, int n2) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "kimiko:shader_hands_bounds_reduce", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0))) {
