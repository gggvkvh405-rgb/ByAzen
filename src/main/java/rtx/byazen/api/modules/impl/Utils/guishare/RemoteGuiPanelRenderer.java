package rtx.byazen.api.modules.impl.Utils.guishare;
...
import rtx.byazen.api.modules.impl.Utils.guishare.GuiShareRemoteState;
import rtx.byazen.api.modules.impl.Utils.guishare.RemoteAvatarCache;
...
import rtx.byazen.api.ui.theme.Theme;
...
public final class RemoteGuiPanelRenderer {
    ...
    private static void renderNoCategoryPlaceholder(DrawContext drawContext, float f, float f2, float f3) {
        ...
        GifRenderer.draw(drawContext, f9, f10, f8, f8, 8.0f, "byazen:gif/kity.gif", f3);
    ...
    private static void renderNameplate(GuiShareRemoteState guiShareRemoteState, RemoteTheme remoteTheme, float f) {
        ...
        float f6 = Fonts.BYAZEN.msdfWidth("x", f3);
        float f7 = f6 + f4 + f5;
        float f8 = 80.0f + (430.0f - f7) * 0.5f;
        float f9 = 88.0f;
        Render2D.rect(f8 - 7.0f, f9 - 7.0f, f7 + 14.0f, 14.0f, 7.0f, RemoteGuiPanelRenderer.color(0, 0, 0, 130, f));
        RemoteGuiPanelRenderer.remoteMsdfIcon(remoteTheme, "byazen", "x", f8, f9 - f3 * 0.5f + 0.5f, f3, 235.0f * f, 0.1f);
    ...
    private static void renderModuleCards(DrawContext drawContext, float f, float f2, RemoteGuiPanel remoteGuiPanel, GuiShareRemoteState guiShareRemoteState, RemoteTheme remoteTheme, List<Module> list, float f3) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (!module.getSettings().all().isEmpty()) {
                ...
                float f43 = Fonts.BYAZEN.msdfWidth(GEAR_GLYPH, 7.0f);
                ...
                Fonts.BYAZEN.msdf(GEAR_GLYPH, f44, f45, 7.0f, n9);
            }
            if (!bl2) continue;
    ...
    private static void renderSidebar(float f, float f2, RemoteGuiPanel remoteGuiPanel, RemoteTheme remoteTheme, float f3) {
        ...
        String string2 = "h";
        float f22 = Fonts.BYAZEN.msdfWidth(string2, 8.0f);
        RemoteGuiPanelRenderer.remoteMsdfIcon(remoteTheme, "byazen", string2, f9, f20 - 4.0f + 1.5f, 8.0f, (200.0f + 55.0f * f21) * f3, 0.15f);
        ...
        for (int i = 0; i < MAIN_CATEGORIES.length; ++i) {
            ...
            float f30 = Fonts.BYAZEN.msdfWidth(string3, 7.0f);
            if (f6 > 0.01f) {
            ...
            RemoteGuiPanelRenderer.remoteMsdfIcon(remoteTheme, "byazen", string3, f25, f29 - 3.5f + 1.5f - f28, 7.0f, (float)n2 * f3, f5);
        ...
        String string5 = "e";
        f6 = Fonts.BYAZEN.msdfWidth(string5, 8.0f);
        RemoteGuiPanelRenderer.remoteMsdfIcon(remoteTheme, "byazen", string5, f9, f32 - 4.0f + 1.5f, 8.0f, (200.0f + 55.0f * f7) * f3, 0.6f);
        ...
        for (int i = 0; i < EVENT_SUBS.length; ++i) {
            ...
            float f37 = Fonts.BYAZEN.msdfWidth(string6, 7.0f);
            if (f4 > 0.01f) {
            ...
            RemoteGuiPanelRenderer.remoteMsdfIcon(remoteTheme, "byazen", string6, f25, f36 - 3.5f + 1.5f - f28, 7.0f, (float)n4 * f3, 0.6f);
        ...
        for (int i = 0; i < OTHER_CATEGORIES.length; ++i) {
            ...
            float f44 = Fonts.BYAZEN.msdfWidth(string7, 7.5f);
            RemoteGuiPanelRenderer.remoteMsdfIcon(remoteTheme, "byazen", string7, f9, f42 - 3.75f + 1.5f - f28, 7.5f, (float)n6 * f3, 0.85f);
    ...
    private static void renderSearchField(DrawContext drawContext, float f, float f2, float f3, float f4, String string, RemoteTheme remoteTheme, float f5) {
        ...
        Fonts.BYAZEN.msdf("q", f8, f9, f7, n3 << 24 | n2);
