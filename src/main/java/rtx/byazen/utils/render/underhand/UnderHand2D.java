package rtx.byazen.utils.render.underhand;
...
import rtx.byazen.api.events.impl.render.UnderHandRenderEvent;
import rtx.byazen.utils.render.others.RenderSampler;
import rtx.byazen.utils.render.render2d.Render2D;
import rtx.byazen.utils.render.render2d.msdf.MsdfFont;
import rtx.byazen.utils.render.render2d.msdf.MsdfFonts;
import rtx.byazen.utils.render.render2d.msdf.MsdfQuadLayout;
...
public final class UnderHand2D {
    ...
    private final ProjectionMatrix2 projection = new ProjectionMatrix2("byazen_underhand", 1000.0f, 11000.0f, true);
    ...
    public void nativeText(Text text, float f, float f2, float f3, int n) {
        ...
        try {
            ...
            ByAzen.LOGGER.error("[UnderHand2D] nativeText failed", throwable);
        }
        finally {
    ...
    private GpuBuffer ensureVertexBuffer(String string, int n) {
        ...
        GpuBuffer gpuBuffer2 = RenderSystem.getDevice().createBuffer(() -> "byazen:underhand_vb_" + string, 40, (long)n2);
    ...
    private void drawPending() {
        ...
        try {
            ...
            ByAzen.LOGGER.error("[UnderHand2D] flush failed", throwable);
            this.discardBuilders();
        }
    }
    ...
    private void drawMesh(CommandEncoder commandEncoder, Framebuffer framebuffer, String string, BuiltBuffer builtBuffer, RenderPipeline renderPipeline, GpuTextureView gpuTextureView, GpuBufferSlice gpuBufferSlice, GpuBufferSlice gpuBufferSlice2) {
        ...
        try {
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:underhand_" + string, framebuffer.getColorAttachmentView(), OptionalInt.empty());){
