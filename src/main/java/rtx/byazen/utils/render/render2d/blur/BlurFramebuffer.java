package rtx.byazen.utils.render.render2d.blur;
...
import rtx.byazen.utils.render.render2d.blur.BlurCapture;
import rtx.byazen.utils.render.render2d.blur.BlurRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltBlur builtBlur) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new BlurRenderState(matrix3x2f, builtBlur2, screenRect, blurCapture));
        }
    ...
    public void preparePending() {
        ...
        if (f >= 0.0f) {
            ...
            this.globalResult = this.ensureTarget(this.globalResult, "byazen_blur_global", n2, n);
        ...
        if (f2 >= 0.0f && blurFramebufferRegion2 != null) {
            ...
            this.worldGlobalResult = this.ensureTarget(this.worldGlobalResult, "byazen_blur_global_world", n3, n);
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_blur_params", 136, 24576L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureKawaseParamsBuffer() {
        if (this.kawaseParamsBuffer == null || this.kawaseParamsBuffer.isClosed() || this.kawaseParamsBuffer.size() < 48L) {
            this.closeKawaseParamsBuffer();
            this.kawaseParamsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_kawase_params", 136, 48L);
        }
    ...
    private void ensureScratchTargets(int n, int n2, int n3, int n4) {
        ...
        for (int i = 0; i < n4; ++i) {
            simpleFramebufferArray[i] = this.ensureTarget(simpleFramebufferArray[i], "byazen_blur_down_" + n + "_" + i, n5, n6);
            if (i < n4 - 1) {
                simpleFramebufferArray2[i] = this.ensureTarget(simpleFramebufferArray2[i], "byazen_blur_up_" + n + "_" + i, n5, n6);
            }
            n5 = Math.max(n5 / 2, 1);
            n6 = Math.max(n6 / 2, 1);
        }
    ...
    private SimpleFramebuffer ensureResultTarget(int n, int n2, int n3) {
        ...
        simpleFramebuffer = this.ensureTarget(simpleFramebuffer, "byazen_blur_result_" + n, n2, n3);
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        if (this.paramsDirty) {
            ...
            try (MemoryStack memoryStack = MemoryStack.stackPush();){
                ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedBlurs);
                this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_blur_params", 136, byteBuffer);
            }
            this.paramsDirty = false;
            return this.paramsBuffer;
        }
    ...
    private GpuBuffer ensureFullscreenQuadBuffer() {
        ...
        try (BufferAllocator bufferAllocator = new BufferAllocator(4 * VertexFormats.POSITION.getVertexSize());){
            ...
            block12: {
                ...
                try {
                    gpuBuffer = this.fullscreenQuadBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_blur_fullscreen_quad", 32, builtBuffer.getBuffer());
    ...
    private void renderKawasePass(RenderPipeline renderPipeline, GpuTextureView gpuTextureView, int n, int n2, SimpleFramebuffer simpleFramebuffer, float f, float f2, float f3, float f4, float f5, GpuSampler gpuSampler) {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen_kawase_blur", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0), null, OptionalDouble.empty());){
