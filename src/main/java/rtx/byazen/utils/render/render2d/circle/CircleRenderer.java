package rtx.byazen.utils.render.render2d.circle;
...
import rtx.byazen.utils.render.render2d.circle.CircleRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltCircle builtCircle) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new CircleRenderState(matrix3x2f, builtCircle2, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_circle_params", 136, 16384L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedCircles);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_circle_params", 128, byteBuffer);
            this.paramsDirty = false;
