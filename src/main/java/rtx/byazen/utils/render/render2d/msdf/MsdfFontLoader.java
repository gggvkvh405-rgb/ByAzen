...
final class MsdfFontLoader {
    ...
    static MsdfFont load(String string) {
        Identifier identifier = Identifier.of("byazen", string + ".json");
        Identifier identifier2 = Identifier.of("byazen", string + ".png");
        ...
        try {
            ...
            ByAzen.LOGGER.warn("[MSDF] Failed to register atlas texture {}", identifier2, throwable);
            return null;
        }
        Optional<Resource> optional = minecraftClient.getResourceManager().getResource(identifier);
        if (optional.isEmpty()) {
            ByAzen.LOGGER.warn("[MSDF] Layout JSON not found: {}", identifier);
            return null;
        }
        try (BufferedReader bufferedReader = optional.get().getReader()) {
            ...
            ByAzen.LOGGER.info("[MSDF] Loaded font {} ({} glyphs, {}x{} atlas)", string, hashMap.size(), n, n2);
            return new MsdfFont(identifier2, n, n2, f2, f3, f4, hashMap, hashMap2);
        }
        catch (Throwable throwable3) {
            ByAzen.LOGGER.warn("[MSDF] Failed to parse layout {}", identifier, throwable3);
            return null;
        }
    }
}
