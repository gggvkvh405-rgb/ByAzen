package rtx.byazen.utils.render.render2d.arc;
...
import rtx.byazen.utils.render.render2d.arc.ArcRenderState;
...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltArc builtArc) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new ArcRenderState(matrix3x2f, builtArc, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_arc_params", 136, 8192L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedArcs);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_arc_params", 128, byteBuffer);
            this.paramsDirty = false;
