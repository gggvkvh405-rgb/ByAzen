package rtx.byazen.utils.render.render2d.rectangle.recthalftone;
...
import rtx.byazen.utils.render.render2d.rectangle.recthalftone.HalftoneRectangleRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltHalftoneRectangle builtHalftoneRectangle) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new HalftoneRectangleRenderState(matrix3x2f, builtHalftoneRectangle2));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_rect_halftone_params", 136, 49152L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedRectangles);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_rect_halftone_params", 128, byteBuffer);
            this.paramsDirty = false;
