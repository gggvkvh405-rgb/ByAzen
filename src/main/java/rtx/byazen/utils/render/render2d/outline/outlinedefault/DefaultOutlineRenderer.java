package rtx.byazen.utils.render.render2d.outline.outlinedefault;
...
import rtx.byazen.utils.render.render2d.outline.outlinedefault.DefaultOutlineRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltOutline builtOutline) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new DefaultOutlineRenderState(matrix3x2f, builtOutline2, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_outline_default_params", 136, 64512L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedOutlines);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_outline_default_params", 128, byteBuffer);
            this.paramsDirty = false;
