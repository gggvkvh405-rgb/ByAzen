package rtx.byazen.utils.render.render2d.outline.outlineglass;
...
import rtx.byazen.utils.render.render2d.outline.outlineglass.GlassOutlineRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltGlassOutline builtGlassOutline) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new GlassOutlineRenderState(matrix3x2f, builtGlassOutline2, ScissorUtil.current(), blurCapture));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_outline_glass_params", 136, 49056L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedOutlines);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_outline_glass_params", 128, byteBuffer);
            this.paramsDirty = false;
