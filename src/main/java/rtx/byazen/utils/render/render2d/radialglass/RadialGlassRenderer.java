package rtx.byazen.utils.render.render2d.radialglass;
...
import rtx.byazen.utils.render.render2d.radialglass.RadialGlassRenderState;
...
implements AutoCloseable {
    ...
    public void submit(DrawContext drawContext, BuiltRadialGlass builtRadialGlass) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new RadialGlassRenderState(matrix3x2f, builtRadialGlass2, ScissorUtil.current(), blurCapture));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_radial_glass_params", 136, 7680L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedSectors);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_radial_glass_params", 128, byteBuffer);
            this.paramsDirty = false;
