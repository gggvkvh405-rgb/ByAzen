package rtx.byazen.utils.render.render2d.glow;
...
import rtx.byazen.utils.render.render2d.GradientSweep;
import rtx.byazen.utils.render.render2d.Render2DCoordinateSpace;
import rtx.byazen.utils.render.render2d.glow.BuiltGlow;
import rtx.byazen.utils.render.render2d.glow.GlowCapture;
import rtx.byazen.utils.render.render2d.glow.GlowRenderState;
import rtx.byazen.utils.render.scissor.ScissorUtil;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltGlow builtGlow) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new GlowRenderState(matrix3x2f, builtGlow, ScissorUtil.current(), glowCapture));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_params", 136, 17920L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureKawaseParamsBuffer() {
        ...
        this.closeKawaseParamsBuffer();
        try {
            this.kawaseParamsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_kawase_params", 136, 48L);
            return this.kawaseParamsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private void ensureScratchTargets(int n, int n2, int n3, int n4) {
        ...
        for (int i = 0; i < n4; ++i) {
            simpleFramebufferArray[i] = this.ensureTarget(simpleFramebufferArray[i], "byazen_glow_down_" + n + "_" + i, n5, n6);
            if (i < n4 - 1) {
                simpleFramebufferArray2[i] = this.ensureTarget(simpleFramebufferArray2[i], "byazen_glow_up_" + n + "_" + i, n5, n6);
            }
            n5 = Math.max(n5 / 2, 1);
            n6 = Math.max(n6 / 2, 1);
        }
    ...
    private SimpleFramebuffer ensureResultTarget(int n, int n2, int n3) {
        ...
        SimpleFramebuffer simpleFramebuffer = this.ensureTarget(this.resultTargets.get(n), "byazen_glow_result_" + n, n2, n3);
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_params", 128, byteBuffer);
            this.paramsDirty = false;
    ...
    private GpuBuffer ensureFullscreenQuadBuffer() {
        ...
        try (BufferAllocator bufferAllocator = new BufferAllocator(4 * VertexFormats.POSITION.getVertexSize());
            ...
            this.fullscreenQuadBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_fullscreen_quad", 32, builtBuffer.getBuffer());
        }
        return this.fullscreenQuadBuffer;
    }
    ...
    private void renderKawasePass(RenderPipeline renderPipeline, GpuTextureView gpuTextureView, int n, int n2, SimpleFramebuffer simpleFramebuffer, float f, float f2, float f3, float f4, float f5, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen_glow_kawase", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0), null, OptionalDouble.empty())) {
    ...
    private SimpleFramebuffer ensureBlurTarget(int n, int n2, int n3) {
        ...
        SimpleFramebuffer simpleFramebuffer = this.ensureTarget(this.blurTargets.get(n), "byazen_glow_blur_" + n, n2, n3);
    ...
    private void renderSourceShape(int n, BuiltGlow builtGlow, GlowCapture glowCapture, SimpleFramebuffer simpleFramebuffer, int n2, int n3, int n4, int n5, int n6, int n7) {
        ...
        try (BufferAllocator bufferAllocator = new BufferAllocator(4 * VertexFormats.POSITION.getVertexSize())) {
            ...
            try (BuiltBuffer builtBuffer = bufferBuilder.end()) {
                gpuBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_src_quad_" + n, 32, builtBuffer.getBuffer());
            }
        } catch (RuntimeException runtimeException) {
            return;
        }
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush()) {
            ...
            byteBuffer.position(0);
            paramsGpuBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_src_params_" + n, 136, byteBuffer);
        } catch (RuntimeException runtimeException) {
            gpuBuffer.close();
            return;
        }
        try {
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen_glow_source_" + n, simpleFramebuffer.getColorAttachmentView(), OptionalInt.empty(), null, OptionalDouble.empty());){
    ...
    private void clearTarget(SimpleFramebuffer simpleFramebuffer) {
        ...
        CommandEncoder commandEncoder = RenderSystem.getDevice().createCommandEncoder();
        RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen_glow_clear", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0), null, OptionalDouble.empty());
        if (renderPass != null) {
            renderPass.close();
        }
    }
    ...
    private void renderCutoutPass(SimpleFramebuffer simpleFramebuffer, SimpleFramebuffer simpleFramebuffer2, SimpleFramebuffer simpleFramebuffer3, GpuSampler gpuSampler) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen_glow_cutout", simpleFramebuffer3.getColorAttachmentView(), OptionalInt.of(0), null, OptionalDouble.empty())) {
    ...
    private SimpleFramebuffer ensureShapeTarget(int n, int n2, int n3) {
        ...
        SimpleFramebuffer simpleFramebuffer = this.ensureTarget(this.shapeTargets.get(n), "byazen_glow_shape_" + n, n2, n3);
    ...
    private void renderShape(int n, BuiltGlow builtGlow, SimpleFramebuffer simpleFramebuffer) {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush()) {
            ...
            GlowRenderer.writePalette(byteBuffer, builtGlow);
            byteBuffer.position(0);
            gpuBuffer2 = RenderSystem.getDevice().createBuffer(() -> "byazen_glow_shape_params_" + n, 136, byteBuffer);
        } catch (RuntimeException runtimeException) {
            return;
        }
        try {
            ...
            RenderSystem.ShapeIndexBuffer shapeIndexBuffer = RenderSystem.getSequentialBuffer((VertexFormat.DrawMode)VertexFormat.DrawMode.QUADS);
            GpuBuffer gpuBuffer3 = shapeIndexBuffer.getIndexBuffer(6);
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen_glow_shape_" + n, simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0), null, OptionalDouble.empty());){
