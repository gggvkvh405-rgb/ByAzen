package rtx.byazen.utils.render.render2d.glass;
...
import rtx.byazen.utils.render.render2d.glass.GlassRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltGlass builtGlass) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new GlassRenderState(matrix3x2f, builtGlass2, ScissorUtil.current(), blurCapture));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glass_params", 136, 57344L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedGlasses);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_glass_params", 128, byteBuffer);
            this.paramsDirty = false;
