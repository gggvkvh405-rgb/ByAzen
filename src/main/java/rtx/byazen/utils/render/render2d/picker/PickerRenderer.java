package rtx.byazen.utils.render.render2d.picker;
...
import rtx.byazen.utils.render.render2d.picker.PickerRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltPicker builtPicker) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new PickerRenderState(matrix3x2f, builtPicker, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_picker_params", 136, 12288L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack, this.preparedPickers);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_picker_params", 128, byteBuffer);
            this.paramsDirty = false;
