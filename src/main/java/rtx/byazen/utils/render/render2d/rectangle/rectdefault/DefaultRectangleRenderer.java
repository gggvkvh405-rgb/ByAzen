package rtx.byazen.utils.render.render2d.rectangle.rectdefault;
...
import rtx.byazen.utils.render.render2d.rectangle.rectdefault.DefaultRectangleRenderState;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltRectangle builtRectangle) {
        ...
        try {
            ...
            ((GuiGraphicsExtractorAccessor)drawContext).byazen_getGuiRenderState().addSimpleElement((SimpleGuiElementRenderState)new DefaultRectangleRenderState(matrix3x2f, builtRectangle2, ScissorUtil.current()));
        }
    ...
    private GpuBuffer ensurePageBuffer(int n) {
        ...
        if (gpuBuffer == null || gpuBuffer.isClosed() || gpuBuffer.size() < 64512L) {
            ...
            try {
                gpuBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_rect_default_params", 136, 64512L);
            }
            catch (RuntimeException runtimeException) {
