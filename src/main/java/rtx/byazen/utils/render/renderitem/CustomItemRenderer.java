package rtx.byazen.utils.render.renderitem;
...
import rtx.byazen.mixin.accessor.ItemLayerRenderStateAccessor;
import rtx.byazen.mixin.accessor.ItemStackRenderStateAccessor;
import rtx.byazen.utils.render.post.GuiRenderStateLayerAccessor;
import rtx.byazen.utils.render.render2d.Render2DCoordinateSpace;
import rtx.byazen.utils.render.render2d.effecticon.PoseKey;
import rtx.byazen.utils.render.renderitem.BuiltRenderItem;
import rtx.byazen.utils.render.renderitem.CachedItemGeometry;
import rtx.byazen.utils.render.renderitem.CachedItemQuad;
import rtx.byazen.utils.render.renderitem.CustomItemRenderState;
import rtx.byazen.utils.render.renderitem.ItemTexture;
...
implements AutoCloseable {
    ...
    private void submit(DrawContext drawContext, BuiltRenderItem builtRenderItem) {
        ...
        try {
            ...
            int n = ((GuiRenderStateLayerAccessor)guiRenderState).byazen_getLayerSerial();
            ...
            ByAzen.LOGGER.warn("[RenderItem] Failed to submit item: {}", (Object)builtRenderItem.stack().getName().getString(), (Object)runtimeException);
        }
    }
    ...
    private GpuBuffer ensureWritableParamsBuffer() {
        ...
        this.closeParamsBuffer();
        try {
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_item_params", 136, 65536L);
            return this.paramsBuffer;
        }
        catch (RuntimeException runtimeException) {
            return null;
    ...
    private GpuBuffer ensureParamsBuffer() {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = this.buildUniformData(memoryStack);
            this.paramsBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen_item_params", 128, byteBuffer);
            this.paramsDirty = false;
    ...
    private CachedItemGeometry buildGeometry(MinecraftClient minecraftClient, ItemStack itemStack, int n) {
        ...
        ItemRenderState.LayerRenderState[] layerRenderStateArray = itemStackRenderStateAccessor.byazen_getLayers();
        int n2 = Math.min(itemStackRenderStateAccessor.byazen_getActiveLayerCount(), layerRenderStateArray.length);
        for (int i = 0; i < n2; ++i) {
    ...
    private Matrix4f layerTransform(ItemLayerRenderStateAccessor itemLayerRenderStateAccessor) {
        ...
        Transformation transformation = itemLayerRenderStateAccessor.byazen_getItemTransform();
    ...
    private void collectLayerGeometry(List<RawItemQuad> list, Bounds bounds, ItemRenderState.LayerRenderState layerRenderState) {
        ...
        ItemRenderState.Glint glint = itemLayerRenderStateAccessor.byazen_getFoilType();
        boolean bl = glint != ItemRenderState.Glint.NONE;
        int[] nArray = itemLayerRenderStateAccessor.byazen_getTintLayers();
        if (nArray == null) {
            nArray = EMPTY_TINTS;
        }
        Matrix4f matrix4f = this.layerTransform(itemLayerRenderStateAccessor);
        boolean bl2 = itemLayerRenderStateAccessor.byazen_getUsesBlockLight();
        if (bl2) {
    ...
    private boolean hasSpecialRenderer(ItemRenderState.LayerRenderState layerRenderState) {
        return layerRenderState != null && ((ItemLayerRenderStateAccessor)layerRenderState).byazen_getSpecialRenderer() != null;
    }
