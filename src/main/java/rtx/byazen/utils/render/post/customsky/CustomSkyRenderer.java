package rtx.byazen.utils.render.post.customsky;
...
public final class CustomSkyRenderer {
    ...
    private static final String[] MARCH_PASS_NAMES = new String[]{"byazen:customsky_march_aurora", "byazen:customsky_march_blackhole", "byazen:customsky_march_starfall"};
    ...
    public static void apply(Framebuffer framebuffer, Matrix4f matrix4f, float f, int n, float f2, float f3, float f4, float f5, float f6, float f7, float f8, float f9) {
        ...
        try {
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:customsky_composite", framebuffer.getColorAttachmentView(), OptionalInt.empty())) {
    ...
    private static void init(int n) {
        ...
        try {
            ...
            if (bloomUniformBuffer == null || bloomUniformBuffer.isClosed() || bloomUniformBuffer.size() < 16L) {
                CustomSkyRenderer.closeBloomUniform();
                bloomUniformBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen:customsky_bloom_uniforms", 136, 16L);
            }
            if (uniformBuffer == null || uniformBuffer.isClosed() || uniformBuffer.size() < 192L) {
                CustomSkyRenderer.closeUniform();
                uniformBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen:customsky_uniforms", 136, 192L);
            }
            if (n == 1) {
    ...
    private static boolean ensureTargets(int n, int n2, int n3) {
        ...
        for (int k = 0; k < 2; ++k) {
            final int texIdx = k;
            CustomSkyRenderer.skyTextures[k] = gpuDevice.createTexture(() -> "byazen:customsky_march" + texIdx, 12, TextureFormat.RGBA8, n6, n7, 1, 1);
            CustomSkyRenderer.skyTextureViews[k] = gpuDevice.createTextureView(skyTextures[k]);
        }
        sceneCopyTexture = gpuDevice.createTexture(() -> "byazen:customsky_scene_copy", 5, TextureFormat.RGBA8, n, n2, 1, 1);
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            n4 = Math.max(1, n4 / 2);
            int n8 = i;
            CustomSkyRenderer.bloomTextures[i] = gpuDevice.createTexture(() -> "byazen:customsky_bloom" + n8, 12, TextureFormat.RGBA8, n5, n4, 1, 1);
    ...
    private static void ensureNoiseTexture() {
        ...
        noiseTexture = gpuDevice.createTexture(() -> "byazen:customsky_noise", 5, TextureFormat.RGBA8, 256, 256, 1, 1);
    ...
    private static void buildBloom(CommandEncoder commandEncoder, GpuTextureView gpuTextureView, boolean bl) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:customsky_bloom" + n3, destView, OptionalInt.empty());){
