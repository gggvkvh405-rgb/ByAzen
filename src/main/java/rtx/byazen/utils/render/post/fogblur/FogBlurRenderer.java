package rtx.byazen.utils.render.post.fogblur;
...
public final class FogBlurRenderer {
    ...
    private static void init() {
        ...
        try {
            ...
            if (compositeUniformBuffer == null || compositeUniformBuffer.isClosed() || compositeUniformBuffer.size() < 48L) {
                FogBlurRenderer.closeCompositeBuffer();
                compositeUniformBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen:fog_blur_uniforms", 136, 48L);
            }
            if (kawaseUniformBuffer == null || kawaseUniformBuffer.isClosed() || kawaseUniformBuffer.size() < 48L) {
                FogBlurRenderer.closeKawaseBuffer();
                kawaseUniformBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen:fog_blur_kawase_uniforms", 136, 48L);
            }
        }
        catch (Throwable throwable) {
            disabledAfterError = true;
    ...
    private static void composite(GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, float f, float f2, float f3, float f4) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:fog_blur_composite", gpuTextureView, OptionalInt.empty());){
    ...
    private static void renderKawasePass(RenderPipeline renderPipeline, GpuTextureView gpuTextureView, int n, int n2, SimpleFramebuffer simpleFramebuffer, float f, float f2, float f3, float f4, float f5, GpuSampler gpuSampler) {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ...
            try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:fog_blur_kawase", simpleFramebuffer.getColorAttachmentView(), OptionalInt.of(0));){
    ...
    private static boolean ensureTargets(int n, int n2, float f, int n3) {
        ...
        FogBlurRenderer.closeTargets();
        sceneCopyTexture = gpuDevice.createTexture(() -> "byazen:fog_blur_scene_copy", 5, TextureFormat.RGBA8, n, n2, 1, 1);
        sceneCopyTextureView = gpuDevice.createTextureView(sceneCopyTexture);
        depthCopyTexture = gpuDevice.createTexture(() -> "byazen:fog_blur_depth_copy", 5, TextureFormat.DEPTH32, n, n2, 1, 1);
        depthCopyTextureView = gpuDevice.createTextureView(depthCopyTexture);
        int n6 = n4;
        int n7 = n5;
        for (int i = 0; i < n3; ++i) {
            FogBlurRenderer.downTargets[i] = FogBlurRenderer.ensureTarget(downTargets[i], "byazen_fog_blur_down_" + i, n6, n7);
            if (i < n3 - 1) {
                FogBlurRenderer.upTargets[i] = FogBlurRenderer.ensureTarget(upTargets[i], "byazen_fog_blur_up_" + i, n6, n7);
            }
            n6 = Math.max(n6 / 2, 1);
            n7 = Math.max(n7 / 2, 1);
        }
    ...
    public static void captureOpaqueDepth(Framebuffer framebuffer) {
        ...
        try {
            ...
            if (opaqueDepthTexture == null || opaqueDepthWidth != framebuffer.textureWidth || opaqueDepthHeight != framebuffer.textureHeight) {
                FogBlurRenderer.closeOpaqueDepth();
                opaqueDepthTexture = gpuDevice.createTexture(() -> "byazen:fog_blur_opaque_depth", 5, gpuTexture.getFormat(), framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
