package rtx.byazen.utils.render.post.handsflame;
...
public final class HandsFlameRenderer {
    ...
    private static void restoreProtectedNameTags(CommandEncoder commandEncoder, Framebuffer framebuffer, GpuTextureView gpuTextureView) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:hands_flame_restore_nametags", framebuffer.getColorAttachmentView(), OptionalInt.empty(), null, OptionalDouble.empty());){
    ...
    private static void ensureTextures(int n, int n2) {
        ...
        beforeTexture = HandsFlameRenderer.createTexture("byazen:hands_flame_before", n, n2, 7);
        sceneTexture = HandsFlameRenderer.createTexture("byazen:hands_flame_scene", n, n2, 5);
        handTexture = HandsFlameRenderer.createTexture("byazen:hands_flame_hand_temp", n, n2, 5);
        trailTextureA = HandsFlameRenderer.createTexture("byazen:hands_flame_trail_a", n3, n4, 13);
        trailTextureB = HandsFlameRenderer.createTexture("byazen:hands_flame_trail_b", n3, n4, 13);
    ...
    private static boolean ensureBlackTexture() {
        ...
        blackTexture = RenderSystem.getDevice().createTexture(() -> "byazen:hands_flame_black", 13, TextureFormat.RGBA8, 1, 1, 1, 1);
    ...
    private static void ensureIrisDepthTextures(Framebuffer framebuffer) {
        ...
        for (int i = 0; i < 2; ++i) {
            HandsFlameRenderer.irisDepthBeforeTextures[i] = RenderSystem.getDevice().createTexture(() -> "byazen:hands_flame_iris_depth_before", 5, textureFormat, framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
            HandsFlameRenderer.irisDepthAfterTextures[i] = RenderSystem.getDevice().createTexture(() -> "byazen:hands_flame_iris_depth_after", 5, textureFormat, framebuffer.textureWidth, framebuffer.textureHeight, 1, 1);
    ...
    private static void renderTrail(CommandEncoder commandEncoder, Framebuffer framebuffer, GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, GpuTextureView gpuTextureView4) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:hands_flame_trail", HandsFlameRenderer.nextTrailView(), OptionalInt.empty(), null, OptionalDouble.empty());){
    ...
    private static void initPipelines() {
        try {
            ...
            uniformBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen:hands_flame_uniform", 136, 128L);
            ByteBuffer byteBuffer = MemoryUtil.memAlloc((int)4);
            byteBuffer.putInt(0);
            byteBuffer.flip();
            dummyVertexBuffer = RenderSystem.getDevice().createBuffer(() -> "byazen:hands_flame_dummy_vertex", 40, byteBuffer);
    ...
    private static void renderComposite(CommandEncoder commandEncoder, Framebuffer framebuffer, GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, GpuTextureView gpuTextureView4, GpuTextureView gpuTextureView5) {
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:hands_flame_composite", framebuffer.getColorAttachmentView(), OptionalInt.empty(), null, OptionalDouble.empty());){
    ...
    private static void disableAfterError(Throwable throwable) {
        ...
        ByAzen.LOGGER.error("Hands flame renderer failed; disabling effect", throwable);
