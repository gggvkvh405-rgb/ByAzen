package rtx.byazen.utils.render.post.guimotionblur;
...
import rtx.byazen.utils.render.others.FullscreenQuadBuffer;
...
public final class GuiMotionBlurRenderer {
    ...
    private static void composite(CommandEncoder commandEncoder, GpuTextureView gpuTextureView, GpuTextureView gpuTextureView2, GpuTextureView gpuTextureView3, GpuTextureView gpuTextureView4, GpuTextureView gpuTextureView5, GpuTextureView gpuTextureView6, float f, float f2, int n, int n2, int n3, int n4, int n5, int n6, float[] fArray, int n7, int n8, int n9, int n10, float f3, float f4, float f5) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:gui_motion_blur_composite", gpuTextureView, OptionalInt.empty(), null, OptionalDouble.empty());){
    ...
    private static boolean ensureReady(int n, int n2, float f) {
        try {
            ...
            if (maskVertexBuffer == null || maskVertexBuffer.isClosed() || maskVertexBuffer.size() < (long)MASK_VERTEX_BYTES) {
                ...
                maskVertexBuffer = gpuDevice.createBuffer(() -> "byazen:gui_motion_blur_mask_vertices", 40, (long)MASK_VERTEX_BYTES);
            }
            if (blurUniformBuffer == null || blurUniformBuffer.isClosed() || blurUniformBuffer.size() < 16L) {
                ...
                blurUniformBuffer = gpuDevice.createBuffer(() -> "byazen:gui_motion_blur_uniform", 136, 16L);
            }
            if (compositeUniformBuffer == null || compositeUniformBuffer.isClosed() || compositeUniformBuffer.size() < 80L) {
                ...
                compositeUniformBuffer = gpuDevice.createBuffer(() -> "byazen:gui_motion_blur_composite_uniform", 136, 80L);
            }
    ...
    private static void gaussianPass(CommandEncoder commandEncoder, GpuTextureView gpuTextureView, SimpleFramebuffer simpleFramebuffer, float f, float f2, int n, int n2, int n3, int n4) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:gui_motion_blur_gaussian", simpleFramebuffer.getColorAttachmentView(), OptionalInt.empty(), null, OptionalDouble.empty())) {
    ...
    private static void ensureTargets(GpuDevice gpuDevice, int n, int n2, float f) {
        ...
        GuiMotionBlurRenderer.closeTargets();
        sceneCopyTexture = gpuDevice.createTexture(() -> "byazen:gui_motion_blur_scene_copy", 5, TextureFormat.RGBA8, n, n2, 1, 1);
        sceneCopyTextureView = gpuDevice.createTextureView(sceneCopyTexture);
        backgroundCopyTexture = gpuDevice.createTexture(() -> "byazen:gui_motion_blur_background_copy", 5, TextureFormat.RGBA8, n, n2, 1, 1);
        backgroundCopyTextureView = gpuDevice.createTextureView(backgroundCopyTexture);
        remoteBackgroundCopyTexture = gpuDevice.createTexture(() -> "byazen:gui_motion_blur_remote_background_copy", 5, TextureFormat.RGBA8, n, n2, 1, 1);
        remoteBackgroundCopyTextureView = gpuDevice.createTextureView(remoteBackgroundCopyTexture);
        maskTarget = new SimpleFramebuffer("byazen_gui_motion_blur_mask", n, n2, false);
        horizontalTarget = new SimpleFramebuffer("byazen_gui_motion_blur_scene_h", n, n2, false);
        verticalTarget = new SimpleFramebuffer("byazen_gui_motion_blur_scene_result", n, n2, false);
        backgroundHorizontalTarget = new SimpleFramebuffer("byazen_gui_motion_blur_bg_h", n, n2, false);
        backgroundVerticalTarget = new SimpleFramebuffer("byazen_gui_motion_blur_bg_result", n, n2, false);
        sourceWidth = n;
        sourceHeight = n2;
        blurWidth = n;
        blurHeight = n2;
    ...
    private static void disableAfterError(Throwable throwable) {
        ByAzen.LOGGER.error("[GuiMotionBlur] disabled after render error", throwable);
        backgroundCaptured = false;
        GuiMotionBlurRenderer.releaseResources();
    ...
    private static void renderMask(CommandEncoder commandEncoder, float[] fArray, int n, int n2, int n3, boolean bl) {
        ...
        try (RenderPass renderPass = commandEncoder.createRenderPass(() -> "byazen:gui_motion_blur_mask", maskTarget.getColorAttachmentView(), OptionalInt.of(0), null, OptionalDouble.empty())) {
            if (n4 > 0) {
