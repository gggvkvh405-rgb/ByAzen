...
vec4 gradientColor(vec2 pos) {
    ...
    vec4 c4 = gradColor4;
    if (flameExtra.y > 0.5) {
        vec2 fragXY = byazenFragXYFromUV(texCoord);
        float base = flameExtra.x;
        c1 = vec4(byazenClientPaletteLoop(base + 0.75, fragXY), 1.0);
        c2 = vec4(byazenClientPaletteLoop(base + 0.5, fragXY), 1.0);
        c3 = vec4(byazenClientPaletteLoop(base + 0.25, fragXY), 1.0);
        c4 = vec4(byazenClientPaletteLoop(base, fragXY), 1.0);
    }
