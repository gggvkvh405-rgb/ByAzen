...
vec4 getGradientColor(vec2 p, float blend) {
    ...
    if (ThemeParams.w > 0.5) {
        ...
        c1 = vec4(kimikoClientPaletteLoop(base, fragXY), 1.0);
        c2 = vec4(kimikoClientPaletteLoop(base + 0.25, fragXY), 1.0);
        c3 = vec4(kimikoClientPaletteLoop(base + 0.5, fragXY), 1.0);
        c4 = vec4(kimikoClientPaletteLoop(base + 0.75, fragXY), 1.0);
    }
