...
implements ConfigSerializable {
    ...
    private boolean auraEnabled() {
        ...
        if (this.attackAura == null && ByAzenClient.getInstance() != null && ByAzenClient.getInstance().getModuleManager() != null) {
            this.attackAura = ByAzenClient.getInstance().getModuleManager().getModules().stream().filter(AttackAuraModule.class::isInstance).map(AttackAuraModule.class::cast).findFirst().orElse(null);
        }
