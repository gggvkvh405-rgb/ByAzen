...
import ru.byazen.misc.ConfigProfile;
import ru.byazen.misc.ConfigRegistry;
import ru.byazen.misc.ConfigStore;
import ru.byazen.misc.ConfigStoreGroup;
import ru.byazen.misc.ConfigStoreGroupLifecycle;
import ru.byazen.misc.ConfigStoreRegistry;
...
import ru.byazen.misc.ThemeConfigStore;
import ru.byazen.misc.UserCacheStore;
...
import ru.byazen.notification.TextNotification;
...
implements ConfigStoreRegistry {
    ...
    public ConfigManager(File clientDirectory, Gson gson, ConfigRegistry configRegistry, KeybindRegistry keybindRegistry) {
        ...
        this.registerStore(new ThemeConfigStore(new File(clientDirectory, "theme.byazen"), gson));
        this.registerStore(new UserCacheStore(new File(clientDirectory, "usercache.byazen"), gson));
        this.registerStore(new FriendListStore(new File(clientDirectory, "friends.byazen"), gson));
        this.registerStore(new BlockEspConfigStore(new File(clientDirectory, "blockesp.byazen"), gson));
        this.registerStore(new StaffNameConfigStore(new File(clientDirectory, "staff.byazen"), gson));
        this.registerStore(new WaypointConfigStore(new File(clientDirectory, "waypoints.byazen"), gson));
        this.registerStore(new MacroConfigStore(new File(clientDirectory, "macros.byazen"), gson));
        this.registerStore(new BlockedSoundStore(new File(clientDirectory, "sounds.byazen"), gson));
        this.registerStore(new PasswordConfigStore(new File(clientDirectory, "autoauth.byazen"), gson));
        this.registerStore(new PotionPresetStore(new File(clientDirectory, "potions.byazen"), gson));
    }
    ...
    public void loadProfile(String name) {
        ...
        this.saveUserCache();
        NotificationCenter notifications = ByAzenClient.getNotificationCenter();
    ...
    public List<String> listProfiles() throws IOException {
        ...
        File[] profileFiles = configDirectory.listFiles((directory, fileName) -> fileName != null && fileName.endsWith(".byazen"));
        ...
        for (File profileFile : profileFiles) {
            ...
            profileNames.add(fileName2.substring(0, fileName2.length() - ".byazen".length()));
        }
        return profileNames;
    }
    ...
    private static String sanitizeProfileName(String requestedName) {
        ...
        if (sanitizedName.endsWith(".byazen")) {
            sanitizedName = sanitizedName.substring(0, sanitizedName.length() - ".byazen".length());
        }
    ...
    private File resolveProfileFile(String name) {
        return new File(this.getConfigDirectory(), this.normalizeProfileName(name) + ".byazen");
    }
    ...
    private void saveUserCache() {
        ...
        try {
            ...
            ByAzenClient.getInstance().getLogger().warn("Failed to save user cache", (Throwable)exception);
        }
    }
}
