...
public enum ByAzenHitParticles {
    CROSS("Cross", 0),
    DOLLAR("Dollar", 1),
    STAR("Star", 2),
    BLOOM("Bloom", 3),
    ...
    private ByAzenHitParticles(String label, int column) {
        this.label = label;
        int left = column * 1064;
        int right = left + 1064;
        this.primarySprite = ByAzenHitParticles.sprite(label, left, 0, right, 1066);
        this.secondarySprite = ByAzenHitParticles.sprite(label, left, 1066, right, 2132);
    }
    ...
    public static ByAzenHitParticles process2(String label) {
        for (ByAzenHitParticles particle : ByAzenHitParticles.values()) {
    ...
    static {
        ATLAS = class_2960.method_60655((String)"byazen", (String)"textures/visuals/particles.png");
        LABELS = (String[])Arrays.stream(ByAzenHitParticles.values()).map(ByAzenHitParticles::getString).toArray(String[]::new);
    }
}
