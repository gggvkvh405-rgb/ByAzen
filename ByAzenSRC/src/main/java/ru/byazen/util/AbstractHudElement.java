...
import ru.byazen.misc.FadeAnimation;
import ru.byazen.misc.HudElementConfig;
...
public abstract class AbstractHudElement {
    ...
    protected AbstractHudElement(String name, BooleanSupplier visibility) {
        this.name = name;
        this.layout = ByAzenClient.getHudLayoutManager().register(name).visibleWhen(visibility);
        this.hudElementConfig = new HudElementConfig(name);
        ByAzenClient.getInstance().getConfigRegistry().register(this.hudElementConfig);
        ByAzenClient.getInventoryHudRegistry().register(this);
